...
class CustomDynamicCache(DynamicCache):
    ...
    def get_seq_length(self, layer_idx: Optional[int] = None) -> int:
        ...
        for idx in range(len(self)):
            max_value = max(
                max_value, self.key_cache[idx].size(
                    -2), self.value_cache[idx].size(-2)
            )
        return max_value
    ...
    if return_probs:
        ...
        if do_sample and top_k != 1 and top_p != 0.0 and temperature != 0.0:
            _logits = top_k_top_p_filtering(
                logits.view(-1, logits.size(-1)) / temperature, top_k=top_k, top_p=top_p
            )
            output_ids = torch.multinomial(_logits.softmax(-1), num_samples=1).view(
                logits.shape[:-1]
            )
            probs = torch.gather(
                all_probs, -1, output_ids.unsqueeze(-1)).squeeze(-1)
        else:
...
def base_generate(
    ...
    past_key_values = CustomDynamicCache.from_legacy_cache(None)  # Cache code
...
def exact_self_speculative_generate(
    model,
    tokenizer,
    input_ids,
    max_new_tokens=10,
    ...
    draft_generate_ids = torch.empty(
        [input_ids.size(0), max_step_draft + 1], dtype=torch.long, device=model.device
    )
    past_key_values = CustomDynamicCache.from_legacy_cache(None)  # Cache code
        ...
        while True:
            ...
            if step == 0:
                ...
                for step_draft in range(max_step_draft):
                    ...
                    if (
                        draft_output_probs.item() < th_stop_draft
                        or step + step_draft + 2 >= max_new_tokens
                    ):
                        break
                ...
                past_key_values.crop(past_key_values_length)
                ...
                max_matched = (
                    (output_ids[:, :-1] !=
                     drafted_input_ids[:, 1:]).cumsum(-1) == 0
                ).sum(-1).item() + 1
                ...
                if max_of_max_matched != max_matched:
                    ...
                    past_key_values.crop(
                        ...
                        + drafted_n_tokens
                        + 1
                        - (max_of_max_matched - max_matched),
                    )
                ...
                if auto_th_stop_draft and step_verify % auto_parameters[0] == 0:
                    tmp_matchness = auto_parameters[1] * (tmp_matchness) + (
                        1 - auto_parameters[1]
                    ) * ((max_matched - 1) / drafted_n_tokens)
                    if tmp_matchness < auto_parameters[2]:
                        ...
                        if drafted_n_tokens == max_step_draft:
                            ...
                            new_th_stop_draft = th_stop_draft - \
                                auto_parameters[3]
                    th_stop_draft = (
                        auto_parameters[4] * th_stop_draft
...
def self_speculative_sample(
    ...
    draft_generate_ids = torch.empty(
        [input_ids.size(0), max_step_draft + 2], dtype=torch.long, device=model.device
    )
    draft_generate_probs = torch.empty(
        [input_ids.size(0), max_step_draft, model.config.vocab_size],
        dtype=torch.float,
        device=model.device,
    )
    past_key_values = CustomDynamicCache.from_legacy_cache(None)  # Cache code
        ...
        while True:
            ...
            if step == 0:
                ...
                for step_draft in range(max_step_draft):
                    ...
                    draft_current_input_ids = draft_output_ids
                    draft_past_key_values = draft_output["past_key_values"]
                    origin_output_probs = torch.gather(
                        draft_output["logits"].softmax(-1),
                        -1,
                        draft_output_ids.unsqueeze(-1),
                    ).squeeze(-1)
                    if (
                        origin_output_probs.item() < th_stop_draft
                        and (1 - random_list[step_draft]) <= th_random_draft
                    ) or step + step_draft + 2 >= max_new_tokens:
                        break
                ...
                observed_r_list = (
                    probs[0, :drafted_n_tokens]
                    / draft_generate_probs[0, :drafted_n_tokens]
                ).cpu()
...
generate_fn_mapping = {
    ...
    "ssd": exact_self_speculative_generate,
...
def infer(
    ...
    **kargs,
    ...
    generate_dict["generated_tokens_length"] = generate_ids[0].shape[0]
...
def clip_input(tokenizer, prompt, task_name, max_new_tokens=512, prompt_shots=""):
    ...
    if len(input_ids[0]) + max_new_tokens >= 4096:
        ...
        input_ids = torch.cat(
            (input_ids[0][:2], input_ids[0][2:-3]
             [:-sample_num], input_ids[0][-3:]),
            dim=0,
        ).unsqueeze(0)
    return input_ids
